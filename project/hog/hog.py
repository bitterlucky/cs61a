...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(0, num_rolls):
        score_this_value = dice()
        score += score_this_value
        if score_this_value == 1:
            is_sow_sad = True
    if is_sow_sad:
        score = 1
    return score
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    oneth_player_score = player_score % 10
    tenth_opponent_score = opponent_score // 10 % 10
    return max(1, 3 * abs(oneth_player_score - tenth_opponent_score))
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_scoring = make_averaged(roll_dice, samples_count)(1, dice)
    max_scoring_num_rolls = 1
    for i in range(2, 11):
        this_scoring = make_averaged(roll_dice, samples_count)(i, dice)
        if this_scoring > max_scoring:
            max_scoring = this_scoring
