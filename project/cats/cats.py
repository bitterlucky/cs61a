...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        all_length = len(typed_words)
        correct_length = 0
        for i in range(min(len(typed_words), len(source_words))):
            if typed_words[i] == source_words[i]:
                correct_length += 1
        return correct_length / all_length * 100
    # END PROBLEM 3
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def f(typed, source, temp_limit):
        if temp_limit < 0:
            return limit 
        elif len(typed) == 0 or len(source) == 0:
            return abs(len(typed) - len(source))
        elif typed[0] == source[0]:
            return f(typed[1:], source[1:], temp_limit)
        else:
            return 1 + f(typed[1:], source[1:], temp_limit - 1)  
...
def minimum_mewtations(typed, source, limit):
    ...
    """
    def f(typed, source, temp_limit):
        if temp_limit < 0: # Base cases should go here, you may add more base cases as needed.
            return abs(limit) + 1 
        # END
        # Recursive cases should go below here
            ...
            add =  f(typed, source[1:], temp_limit - 1) + 1# Fill in these lines
            remove = f(typed[1:], source, temp_limit - 1) + 1
            substitute = f(typed[1:], source[1:], temp_limit - 1) + 1
            # BEGIN
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            this_time = time(match, j, i)
            if this_time < min_time:
                min_time = this_time
                min_index = j
        value[min_index].append(get_word(match, i))
    return value
    # END PROBLEM 10
