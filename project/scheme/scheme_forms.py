...
def do_define_form(expressions, env):
    ...
    # expressions = ((f x) (+ x 8))
    #expressions = Pair(Pair(f, Pair(x))  , Pair())
...
def do_if_form(expressions, env):
    ...
    3
    """
    #Pair(#t, Pair(print 2, Pair(print 3, nil)))
...
def do_and_form(expressions, env):
    ...
    val = ''
    while temp != nil:
        this_value = scheme_eval(temp.first, env)
        if is_scheme_true(this_value):
            temp = temp.rest
            val = this_value
        elif is_scheme_false(this_value):
            val = this_value
            break
    return val
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    #Pair(Pair(x, Pair(3),nil), Pair())  ((x 3) (y 1))
    ...
    while temp != nil:
        ...
        vals = Pair(eval_all(temp.first.rest, env), vals)
        temp = temp.rest
    # END PROBLEM 14
