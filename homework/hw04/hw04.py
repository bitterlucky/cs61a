...
def balanced(m):
    ...
    if is_planet(m):
        return True
    left_rod_length = length(left(m))
    right_rod_length = length(right(m))
    left_mass = total_mass(end(left(m)))
    right_mass = total_mass(end(right(m)))
    return left_rod_length * left_mass == right_rod_length * right_mass and balanced(end(left(m))) and balanced(end(right(m)))
